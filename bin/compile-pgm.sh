...
#   Source member : {PGM}
#
# Use --sql for SQLRPGLE source (uses CRTSQLRPGI instead of CRTBNDRPG)
#
# Usage:
#   compile-pgm.sh <PGM> [-e environment] [-l library] [-f file] [-s srcmbr] [-b bnddir] [--sql]
#
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        -b|--bnddir)      BND_DIR="$2"; shift 2 ;;
        --sql)            SQL_PGM=true; shift ;;
...
if [[ -z "$PGM" ]]; then
    echo "Usage: compile-pgm.sh <PGM> [-e environment] [-l library] [-f file] [-s srcmbr] [-b bnddir] [--sql]"
    exit 1
fi
...
echo "=== Compiling $PGM Program ==="
echo "Environment: $ENV_NAME  Library: $LIBRARY"

if $SQL_PGM; then
    if [[ -n "$BND_DIR" ]]; then
        echo "Note: -b/--bnddir ignored for --sql; specify BndDir in Ctl-Opt in source."
    fi
    if ! run_remote "system 'CRTSQLRPGI OBJ($LIBRARY/$PGM) SRCFILE($LIBRARY/$FILE) SRCMBR($RESOLVED_SRC_MBR) OBJTYPE(*PGM) RPGPPOPT(*LVL2) DBGVIEW(*SOURCE)'"; then
        exit 1
    fi
else
    BND_DIR_PARAM=""
    [[ -n "$BND_DIR" ]] && BND_DIR_PARAM=" BNDDIR($BND_DIR)"
    if ! run_remote "system 'CRTBNDRPG PGM($LIBRARY/$PGM) SRCFILE($LIBRARY/$FILE) SRCMBR($RESOLVED_SRC_MBR) DBGVIEW(*SOURCE)$BND_DIR_PARAM'"; then
        exit 1
    fi
fi
