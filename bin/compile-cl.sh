...
#   Source member : {PGM}
#
# Use --ile for CLLE source (uses CRTBNDCL instead of CRTCLPGM)
#
# Usage:
#   compile-cl.sh <PGM> [-e environment] [-l library] [-s srcmbr] [--ile]
#
...
if [[ -z "$PGM" ]]; then
    echo "Usage: compile-cl.sh <PGM> [-e environment] [-l library] [-f file] [-s srcmbr] [--ile]"
    exit 1
fi
...
echo "=== Compiling $PGM CL Program ==="
echo "Environment: $ENV_NAME  Library: $LIBRARY"

if $ILE_CL; then
    if ! run_remote "system 'CRTBNDCL PGM($LIBRARY/$PGM) SRCFILE($LIBRARY/$FILE) SRCMBR($RESOLVED_SRC_MBR) DBGVIEW(*SOURCE)'"; then
        exit 1
    fi
else
    if ! run_remote "system 'CRTCLPGM PGM($LIBRARY/$PGM) SRCFILE($LIBRARY/$FILE) SRCMBR($RESOLVED_SRC_MBR)'"; then
        exit 1
    fi
fi
