...
#
# Usage:
#   run-tests.sh <TESTPROGRAM> [-e environment] [-l library] [-p testproc]
#
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        -p|--testproc)    TEST_PROC="$2"; shift 2 ;;
...
if [[ -z "$TEST_PROGRAM" ]]; then
    echo "Usage: run-tests.sh <TESTPROGRAM> [-e environment] [-l library] [-p testproc]"
    exit 1
fi
...
echo "=== Running Test Suite: $TEST_PROGRAM ==="
echo "Environment: $ENV_NAME  Library: $LIBRARY"
[[ -n "$TEST_PROC" ]] && echo "Test procedure: $TEST_PROC"

TST_PRC_CLAUSE=""
[[ -n "$TEST_PROC" ]] && TST_PRC_CLAUSE=" TSTPRC($TEST_PROC)"
FULL_CMD="qsh -c \"$LIB_LIST_CMDS; system 'RPGUNIT/RUCALLTST TSTPGM($LIBRARY/$TEST_PROGRAM)$TST_PRC_CLAUSE'\""
set +e
...
if [[ $STATUS -ne 0 ]]; then
    echo ""
    echo "ERROR: Test run failed with exit code $STATUS"
    exit 1
fi

echo ""
echo "=== Test Run Complete: $TEST_PROGRAM ==="
