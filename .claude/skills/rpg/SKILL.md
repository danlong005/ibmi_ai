...
- Program naming: `<prefix><seq><suffix>` — e.g. `acmr100`, `aer021`, `mbrmgr`
...
Ctl-Opt NoMain BndDir('LGLSHIELD' : 'YAJL' : 'HTTPAPI');
...
// COALESCE with config table (project pattern)
Exec SQL SET :ConfigVal = COALESCE(
  (SELECT GLOVAL FROM GLOBALCFG
    WHERE GLOAPP = :AppName AND GLOKEY = :KeyName
...
Dcl-Pr ENC9010R ExtPgm('ENC9010R');
  RawSSN   Char(9) Const;
  EncSSN   Char(50);
End-Pr;
```
...
Include with: `/Include ILESRC,MYMOD_H`
...
## HTTP / REST (HTTPAPI + YAJL — project pattern)
...
```rpgle
Ctl-Opt NoMain BndDir('LGLSHIELD':'UTILBD');
...
## SSN / PII encryption (project-specific, CRITICAL)

SSN must **always** be encrypted. Never store a raw SSN in `AENSSN`.

```rpgle
// Encrypt SSN via SQL function (preferred)
Exec SQL UPDATE ENROLLTBL
  SET AENSSN = ENCRYPTSSN(:RawSSN)
  WHERE MEMBERID = :MemberId;

// Or via ENC9010R program call
/Include ILESRC,SYNC_H
Dcl-S EncSSN Char(50);
ENC9010R(RawSSN : EncSSN);
```
...
| `x'F6'` | F6 (Add) |
| `x'FC'` | F12 (Cancel) |
| `x'F1'` | Enter (x'F1' with RSTDSP) |
| `x'31'` | Enter |
| `x'32'` | F2 (actually varies — check INFDS) |
