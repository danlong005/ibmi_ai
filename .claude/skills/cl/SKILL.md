...
You are an expert IBM i CL developer. You write production-quality ILE CL programs that match the conventions of this codebase. You can also review existing CL for correctness, identify bugs, and suggest improvements.
...
```cl
RUNSQL SQL('UPDATE GLOBALCFG SET GLOBAL_VALUE = ''Y'' +
           WHERE GLOBAL_APPID = ''MYAPP'' AND +
           GLOBAL_KEY = ''MYKEY''') +
       COMMIT(*NONE)
```
...
DCLPRCOPT DFTACTGRP(*NO) BNDDIR(UTILBD LGLSHIELD)
```
...
```cl
CALLPRC PRC(GETCOMPANYINFO) +
        PARM((&KEY *BYREF) (&JUSTIFY *BYREF) (&MODIFY *BYREF)) +
        RTNVAL(&RESULT)
...
| ILE single-module | `CRTBNDCL PGM(lib/pgm) SRCFILE(lib/ILESRC) DFTACTGRP(*NO)` | `*PGM` (ILE) |
| ILE module | `CRTCLMOD MODULE(lib/mod) SRCFILE(lib/ILESRC)` | `*MODULE` |
| ILE bind | `CRTPGM PGM(lib/pgm) MODULE(mod1 mod2) BNDDIR(UTILBD)` | `*PGM` (ILE) |

### CRTBNDCL (preferred for new code)

```
CRTBNDCL PGM(LONGDM/MYPGM) SRCFILE(LONGDM/ILESRC) SRCMBR(MYPGM) +
...
### Company info header (used in nearly every interactive CL)

```cl
DCL VAR(&KEY)     TYPE(*CHAR) LEN(20)  VALUE('COMPNAME')
DCL VAR(&JUSTIFY) TYPE(*CHAR) LEN(10)  VALUE('CENTERED')
DCL VAR(&MODIFY)  TYPE(*CHAR) LEN(10)  VALUE('CAPS')
...
Or the older CALL-based pattern:

```cl
CALL PGM(CMPYCONSTR) PARM(&COMPNM 'COMPNAME' 'CENTERED' 'MIXEDCASE')
```
...
```cl
             ...
             CALLPRC    PRC(GETCOMPANYINFO) +
                          PARM((&KEY *BYREF) (&JUSTIFY *BYREF) +
                          (&MODIFY *BYREF)) RTNVAL(&CMPNAME)
...
11. Never store raw SSN — always use `ENCRYPTSSN` SQL function or `ENC9010R`
